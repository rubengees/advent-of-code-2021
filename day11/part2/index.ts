...
function* loopSteps(matrix: number[][]): Generator<{ flashes: number; step: number }> {
  for (let i = 1; ; i++) {
    yield { flashes: step(matrix), step: i }
  }
}

export default function day11_part2(input: string[]): number {
  const matrix: number[][] = []
  ...
  for (const { step, flashes } of loopSteps(matrix)) {
    if (flashes === matrixSize) {
      return step
    }
  }
